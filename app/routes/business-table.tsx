...
import { apiFetch, apiJson, apiBlob, apiUploadFile } from "~/lib/api-client";
...
export default function BusinessTablePage() {
  ...
  useEffect(() => {
    ...
    apiJson<TableSchema>(
      ...
      .catch((e) => {
        ...
        setError(/abort|timeout/i.test(msg) ? "Request timed out. Check the backend and try again." : msg);
      })
      .finally(() => {
  ...
  const handleAddSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      setError(/abort|timeout/i.test(msg) ? "Request timed out. Try again." : msg);
    } finally {
      clearTimeout(timeoutId);
    }
  };
  ...
  const handleImportExcel = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    setError(null);
    try {
      const result = await apiUploadFile<{ created: number; errors: { row: number; errors: Record<string, string> }[] }>(
              ...
              <table className="w-full text-sm">
                ...
                <tbody>
                  {rowsLoading && rows.length === 0 ? (
                    <tr>
                      <td colSpan={schema.fields.length + (canEdit ? 1 : 0)} className="px-4 py-8 text-center text-muted-foreground text-sm">
                        Loading rows…
                      </td>
                    </tr>
                  ) : (
