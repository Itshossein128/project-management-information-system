...
Monorepo (pnpm workspace): `apps/web` (React Router 7 + Vite frontend) and `apps/api` (Django 4.2 + DRF backend — Verona Sprint 1–3+ domains; see `docs/مهندسی و معماری نرم‌افزار (Engineering & Blueprint)/معماری و نقشه محدوده سیستم (Architecture & Scope Map)/نقشه محدوده سیستم Verona (Verona Scope Map).md`). Standard commands live in `README.md` / `package.json` scripts; this section only covers non-obvious cloud caveats. The update script already runs `pnpm install` (against the npmjs registry), creates the API venv at `apps/api/.venv`, installs `requirements.txt` plus the pytest tooling (see below), creates `.env` from `.env.example` (with the local-dev overrides applied), and pins Django back to 4.2.11.
