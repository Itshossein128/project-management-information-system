# Velora — Customer PC deployment (Docker)

Run Velora on a customer PC without cloud hosting. **Double-click** launcher scripts — no terminal commands needed on Windows.
...
Pin a desktop shortcut to `start.bat` so they can launch Velora with one click later.
