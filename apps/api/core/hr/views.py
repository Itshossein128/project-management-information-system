...
class BaseHRRequestViewSet(ProjectScopedViewSet):
    ...
    def supervisor_approve(self, request, project_pk=None, pk=None):
        ...
        obj = self._supervisor_approve(obj, request.user, approved, request.data)
        return Response(self.get_serializer(obj).data)
    ...
    def manager_approve(self, request, project_pk=None, pk=None):
        obj = self.get_object()
        approved = request.data.get('approved', True)
        obj = self._manager_approve(obj, request.user, approved, request.data)
        return Response(self.get_serializer(obj).data)
    ...
    def _supervisor_approve(self, obj, user, approved: bool, data: dict):
        raise NotImplementedError

    def _manager_approve(self, obj, user, approved: bool, data: dict):
        raise NotImplementedError
...
class OvertimeRequestViewSet(BaseHRRequestViewSet):
    ...
    def _supervisor_approve(self, obj, user, approved: bool, data: dict):
        ...
        return services.supervisor_approve_overtime(obj, user, approved, notes)

    def _manager_approve(self, obj, user, approved: bool, data: dict):
        approved_hours = data.get('approved_hours')
        return services.manager_approve_overtime(obj, user, approved, approved_hours)
...
class LeaveRequestViewSet(BaseHRRequestViewSet):
    ...
    def _supervisor_approve(self, obj, user, approved: bool, data: dict):
        return services.supervisor_approve_leave(obj, user, approved)

    def _manager_approve(self, obj, user, approved: bool, data: dict):
        return services.manager_approve_leave(obj, user, approved)
