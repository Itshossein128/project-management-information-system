...
function OfficerDashboardContent() {
  ...
  const { startTour } = useProductTour({
    tourId: "procurement-officer",
    steps: [
      {
        element: "[data-tour='officer-summary']",
        popover: {
          title: t("tour.procurementOfficer.step1Title"),
          description: t("tour.procurementOfficer.step1Desc"),
        },
      },
      {
        element: "[data-tour='officer-cards']",
        popover: {
          title: t("tour.procurementOfficer.step2Title"),
  ...
  return (
    <div className="space-y-6">
      ...
      <p className="text-sm text-muted-foreground" data-tour="officer-summary">
        وضعیت ردیف‌های تخصیص‌یافته به کارپردازان (Line-item Tracking)
      </p>

      <div data-tour="officer-cards">
        {summaries.length === 0 ? (
                  ...
                  <div>
                    <h4 className="font-semibold">
                      {s.assigned_to__full_name ||
                        `${s.assigned_to__first_name || ""} ${s.assigned_to__last_name || ""}`.trim() ||
                        s.assigned_to}
                    </h4>
