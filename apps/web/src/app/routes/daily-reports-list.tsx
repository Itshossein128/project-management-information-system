...
import { CloudOff, Eye, FileText, Pencil, Plus, RefreshCw, Trash2 } from "lucide-react";
...
export default function DailyReportsListPage() {
  ...
  const [filters, setFilters] = useState<DailyReportListParams>({ page: 1, per_page: 20 });
  ...
  const syncNow = async () => {
    ...
    try {
      ...
      if (result.conflicts > 0) toast.error(`${result.conflicts} تعارض شناسایی شد`);
      await queryClient.invalidateQueries({ queryKey: ["daily-reports", projectId] });
      await loadOffline();
    } catch (e) {
  ...
  const stats = useMemo(() => {
    ...
    for (const r of rows) {
      ...
      else if (r.status === "submitted" || r.status === "under_review") s.pending++;
  ...
  const removeMutation = useMutation({
    ...
    onSuccess: () => {
      ...
      void queryClient.invalidateQueries({ queryKey: ["daily-reports", projectId] });
    },
  ...
  return (
    <main className="page-main page-shell mx-auto max-w-6xl px-4 py-6">
      <Breadcrumb
        items={[
          { label: "پروژه", href: `/${PATHS.PROJECT}/${projectId}/${PATHS.PROJECT_OVERVIEW}` },
      ...
      <div className="mb-6 grid gap-4 sm:grid-cols-2 lg:grid-cols-4">
        <StatCard label="کل گزارش‌ها" value={stats.total} tone="text-foreground" />
        <StatCard label="تأیید شده" value={stats.approved} tone="text-emerald-600" />
        <StatCard label="در انتظار" value={stats.pending} tone="text-amber-600" />
        <StatCard label="پیش‌نویس" value={stats.draft} tone="text-muted-foreground" />
      </div>
      ...
      <div className="mb-4 grid gap-3 rounded-xl border border-border bg-card p-4 sm:grid-cols-3">
        ...
        <div>
          ...
          <select
            ...
            onChange={(e) =>
              setFilter({ status: (e.target.value || undefined) as ReportStatus | undefined })
            }
          >
          ...
          <table className="w-full min-w-[760px] text-sm">
            <thead>
              <tr className="bg-muted/50 text-muted-foreground">
                ...
                <th className="px-3 py-2 text-right font-medium">وضعیت کارگاه</th>
            ...
            <tbody>
              ...
              {rows.map((r) => {
                const canEditRow = r.status === "draft" || r.status === "rejected";
                return (
                  <tr key={r.report_id} className="border-t border-border hover:bg-muted/20">
                    <td className="px-3 py-2">{formatDisplayDate(r.report_date)}</td>
                    <td className="px-3 py-2">{r.day_of_week}</td>
                    <td className="px-3 py-2">
                      <Badge
                        variant={r.site_status === "active" ? "success" : "neutral"}
                    ...
                    <td className="px-3 py-2 text-center">{r.activity_count}</td>
                    <td className="px-3 py-2 text-center">{r.labor_total}</td>
                    <td className="px-3 py-2">{r.prepared_by_name ?? "—"}</td>
                    <td className="px-3 py-2">
                      <Badge variant={STATUS_BADGE[r.status]} label={r.status_label} />
                    </td>
                    <td className="px-3 py-2">
                      <div className="flex items-center justify-center gap-1">
                        ...
                        {canEdit && r.status === "draft" ? (
                          <button
                            ...
                            title="حذف"
                            onClick={() => {
                              if (confirm("این گزارش حذف شود؟")) removeMutation.mutate(r.report_id);
                            }}
      ...
      {total > (filters.per_page ?? 20) ? (
        <div className="mt-4 flex items-center justify-center gap-2">
          ...
          <span className="text-sm text-muted-foreground">صفحه {filters.page ?? 1}</span>
          <button
            type="button"
