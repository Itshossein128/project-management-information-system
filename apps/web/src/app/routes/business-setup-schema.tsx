...
export default function BusinessSetupSchema() {
  ...
  const saveTable = async (e: React.FormEvent) => {
    ...
    if (!validateSlug(slug)) {
      setFormError(
        "Slug: lowercase letter first, then letters, numbers, underscores.",
      );
      return;
    }
                ...
                <form onSubmit={saveTable} className='space-y-3'>
                        ...
                        setTableForm(
                          (p) =>
                            p && {
                              ...p,
                              name: e.target.value,
                              slug: p.slug || slugFromName(e.target.value),
                            },
                        )
                      }
                  ...
                  <div>
                    ...
                    <Input
                      ...
                      onChange={(e) =>
                        setTableForm((p) => p && { ...p, slug: e.target.value })
                      }
                      placeholder='e.g. inventory'
