...
### Source Files Analysis — Velora Module Mapping

---

## Summary Table

| # | File | Persian Name | Module in Velora | Type |
|---|---|---|---|---|
...
- In Velora, this becomes a filtered view of `daily_activities` where `subcontractor = 'خنکا'`
...
- Weather condition values should be an enum/dropdown in Velora
...
- The `responsible_party`, `severity`, and `corrective_action` fields are missing in the Excel — they need to be added in the Velora form
- These entries are currently free-text. Velora should provide a category dropdown (equipment failure / payment delay / design change / weather / subcontractor / other) to enable reporting
...
- In Velora, this becomes an auto-generated report from the daily labor entries — not a separate manual input
...
- Productive hours = (end time − start time) − repair hours. Validate this in Velora rather than making the user calculate it.
...
- One row per job title per day. In Velora this is entered as a grid inside the daily report form
...
- The column structure difference in ACTIVITY between phases indicates the customer used evolving Excel templates. Velora must standardize this into a single consistent form
...
This is the most important file — it defines the complete structure of the daily report that Velora must replicate as a digital form.
...
**Notes for Velora digital form:**
- This section is the most data-rich — typically 15–25 activities per day
- Each row maps to one `daily_activities` record
- The activity description is free-text but should be linkable to a WBS activity in Velora
- `نام پیمانکار` is free-text in Excel; in Velora it should be a FK to the subcontractors table
- The form should allow attaching site photos to individual activity rows

**Velora Implementation Notes — Complete Daily Report:**
...
- The `درصد اجرا` (execution percentage) column is unique to this file — not present in the main daily report. Include it in the Velora civil sub-report form.
...
- In Velora this becomes: one `material` record + multiple `purchase_requests` linked to it
...
- In Velora this is computed automatically from `inventory_transactions.unit_cost`
...
**Overall Notes for Velora Material Module:**
...
- Velora should enforce: `start_time` and `end_time` are both required if duration is entered
- The "Email Address" column (Google Forms) is replaced by user authentication in Velora
...
- `همکار جایگزین` should be a FK to `users` — a required field in Velora (currently unenforced in Excel)
- Site security approval (`تایید حراست کارگاه`) should be a separate approval step triggered only when request type = `ماموریت`
- Start and end should be separate date + time fields (not concatenated) in Velora to avoid the data quality issues seen here
...
## Cross-File Relationships & Velora Module Mapping
